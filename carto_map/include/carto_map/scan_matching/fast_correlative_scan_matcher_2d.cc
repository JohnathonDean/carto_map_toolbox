...
PrecomputationGrid2D::PrecomputationGrid2D(
    ...
    const cartographer::mapping::CellLimits& limits, const int width,
...
uint8_t PrecomputationGrid2D::ComputeCellValue(const float probability) const {
  const int cell_value = cartographer::common::RoundToInt(
...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  ...
  std::vector<float> reusable_intermediate_grid;
  const cartographer::mapping::CellLimits limits = grid.limits().cell_limits();
...
bool FastCorrelativeScanMatcher2D::Match(
    ...
    const cartographer::sensor::PointCloud& point_cloud, const float min_score, float* score,
...
bool FastCorrelativeScanMatcher2D::MatchFullSubmap(
  ...
  const SearchParameters search_parameters(
      ...
      point_cloud, limits_.resolution());
  const cartographer::transform::Rigid2d center = cartographer::transform::Rigid2d::Translation(
